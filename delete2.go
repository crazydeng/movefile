...
func LoadKeyword(filepath string) ([]string, error) {
	kw := make([]string, 0)
...
func CheckDelete(filepath string, list []string) bool {

	// list need to remove \n
	for _, kw := range list {
		//fmt.Printf("compare %s:%s\n", filepath, kw)
		if strings.Contains(filepath, kw) {
			return true
		}
	}
...
func main() {
	...
	var check bool
	var keyword_file string

	flag.StringVar(&source_dir, "source", "", "source dir path,default null")
	flag.StringVar(&keyword_file, "keyword", "./kw", "keyword file path,default ./kw")
	...
	keywords, err := LoadKeyword(keyword_file)
	if err != nil {
		log.Fatalf("load key work file error :%s", err.Error())
	}
	fmt.Printf("keyword count:%d\n", len(keywords))
	...
	for _, file := range sourceFiles {
		...
		//fmt.Printf("check file name :%s\n", file.FullPath)

		if !ToDelete(file.FullPath) && !CheckDelete(file.FullPath, keywords) {
			continue
		}

		//fmt.Println("never go")
